...
bool CSearchView::ExportResults(const std::wstring& filePath) {
    ...
    // Helper to escape CSV values (enclosing in quotes and doubling internal quotes)
    auto escapeVal = [isCsv](const std::wstring& val) -> std::wstring {
        if (!isCsv) return val;
        // If it contains comma, quotes, or newlines, escape it
        if (val.find(L',') != std::wstring::npos || val.find(L'"') != std::wstring::npos || 
            val.find(L'\n') != std::wstring::npos || val.find(L'\r') != std::wstring::npos) {
    ...
    fwprintf(file, L"%s%c%s%c%s%c%s%c%s\n", 
             escapeVal(L"Name").c_str(), sep,
             escapeVal(L"Path").c_str(), sep,
             escapeVal(L"Size").c_str(), sep,
...
void CSearchView::SetViewMode(int mode) {
    ...
    // Explicitly arrange icons for non-report styles
    DWORD dwStyle = m_listView.GetWindowLong(GWL_STYLE);
...
LRESULT CSearchView::OnCustomDraw(int idCtrl, LPNMHDR pnmh, BOOL& bHandled) {
    ...
    switch (pLVCD->nmcd.dwDrawStage) {
        ...
        case CDDS_SUBITEM | CDDS_ITEMPREPAINT: {
            ...
            // Set text color and background brush depending on selection and hover/hot state
            bool isSelected = (m_listView.GetItemState(itemIdx, LVIS_SELECTED) & LVIS_SELECTED) != 0;
            bool isFocused = (::GetFocus() == m_listView.m_hWnd);
            bool isHot = (m_listView.GetHotItem() == itemIdx);
            ...
            if (flag == COL_NAME) {
                ...
                RECT rc = pLVCD->nmcd.rc; // logical coordinates are already scroll-origin shifted!

                // Clear background of the cell manually for the Name column
                ...
                for (const auto& seg : segments) {
                    ...
                    // Force segment to clip at Name column bounds
                    if (rcSeg.right > rc.right - 4) {
                        rcSeg.right = rc.right - 4;
                    }
