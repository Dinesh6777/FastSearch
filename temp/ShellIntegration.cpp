...
namespace AppShell {
    ...
    bool ShellIntegration::ShowContextMenu(HWND hwndParent, const std::vector<std::wstring>& filePaths, POINT ptScreen) {
        ...
        // Remove duplicate native Cut, Copy, Delete, Copy path, and Copy as path entries at the bottom of the menu
        ...
        for (int i = itemCount - 1; i >= 12; --i) {
            ...
            if (::GetMenuStringW(hMenu, i, buf, 256, MF_BYPOSITION) > 0) {
                ...
                if (clean == L"cut" || clean == L"copy" || clean == L"delete" || 
                    clean == L"copy as path" || clean == L"copy path") {
        ...
        if (cmdId >= 0x6001 && cmdId <= 0x6008) {
            ...
            else if (cmdId == 0x6003) { // Copy full name to clipboard (absolute path)
                std::wstring text;
